...
from mxnet.ndarray import _internal
...
def _get_interp_method(interp, sizes=()):
    ...
    interp : int
        ...
        0: Nearest Neighbors Interpolation.
        1: Bilinear interpolation.
        2: Bicubic interpolation over 4x4 pixel neighborhood. (used by default).
...
def resize(src, size, interp=2):
    """Resizes both edges to size.

    .. note:: `resize` uses OpenCV (not the CV2 Python library).
       MXNet must have been built with OpenCV for `resize` to work.

    Resizes the original image by setting both edges to size
    Resizing function is called from OpenCV.
    ...
    src : NDArray
        The original image.
    size : int
        The length to be set for both edges.
    interp : int, optional, default=2
        Interpolation method used for resizing the image.
        Possible values:
        0: Nearest Neighbors Interpolation.
...
def rand_resize(src, sizeRange, interp=2):
    """Resizes both edges to size.
    ...
    """
    h, w, _ = src.shape
    new_h = random.randint(sizeRange[0], sizeRange[1])
    new_w = random.randint(sizeRange[0], sizeRange[1])
    #print("rand_resize:" + str(new_h) + ", " + str(new_w))
    # if h > w:
    #     new_h, new_w = size * h // w, size
    # else:
    #     new_h, new_w = size, size * w // h
...
class ResizeAug(Augmenter):
    ...
    def __call__(self, src):
        ...
        return resize(src, self.size, self.interp)

class MyRandResizeAug(Augmenter):
    """Randomly resize input image.
    ...
    interp : int, optional, default=2
        Interpolation method. See resize_short for details.
    """
    def __init__(self, sizeRange, interp=2):
        super(MyRandResizeAug, self).__init__(sizeRange=sizeRange, interp=interp)
    ...
    def __call__(self, src):
        """Augmenter body"""
        #return resize_short(src, self.size, self.interp)
        return rand_resize(src, self.sizeRange, self.interp)
...
def CreateAugmenter(data_shape, resize=0, rand_crop=False, my_rand_resize = False, rand_resize=False, rand_mirror=False,
    ...
    if (type(my_rand_resize) is list):
        auglist.append(MyRandResizeAug(my_rand_resize, inter_method))
